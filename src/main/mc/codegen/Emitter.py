...
class Emitter():
    ...
    def emitCLINIT(self,classname,in_,frame):
        ...
        for x in in_:
            initGlobalArray.append(self.emitINITGLOBALARRAY(classname+"."+x.variable,x.varType,frame))
    ...
    *    @param typ the type of the constant
    ...
    def emitASTORE(self, in_, frame):
        ...
        elif type(in_) is ArrayPointerType or type(in_) is cgen.ClassType  or type(in_) is ArrayType or type(in_) is StringType:
    ...
    *   @param toLabel the ending label  of the scope where the variable is active.
    ...
    def emitREADVAR(self, name, inType, index, frame):
        ...
        if type(inType) is BoolType:
            return self.jvm.emitILOAD(index)
        elif type(inType) is ArrayPointerType or type(inType) is cgen.ClassType  or type(inType) is ArrayType or type(inType) is StringType:
    ...
    *   @param isStatic <code>true</code> if the method is static; <code>false</code> otherwise.
    ...
    def emitNEWARRAY(self,in_,frame):
        ...
        buffer = []
        buffer.append(self.emitPUSHICONST(in_.dimen, frame))
    ...
    *   @param in the type of the returned expression.
    ...
    def emitRETURN(self, in_, frame):
        ...
        if type(in_) is IntType:
            ...
            frame.pop()
            return self.jvm.emitIRETURN()
        elif  (type(in_) is ArrayType) or (type(in_) is ArrayPointerType) or (type(in_) is StringType):
            frame.pop()
